...
class Jogo:
    ...
    def desenha(self):
        ...
        if self.status != 'Конец игры':

            surface_texto, rect_texto = text_objects("Счёт черных: " + str(12 - o), fonte, VERMELHO_CLARO)
            rect_texto.center = (650, 51)
            display.blit(surface_texto, rect_texto)

            surface_texto, rect_texto = text_objects("Счёт белых: " + str(12 - x), fonte, BRANCO)
            rect_texto.center = (650, ALTURA - 30)
            display.blit(surface_texto, rect_texto)

            if self.turno % 2 == 1:
                surface_texto, rect_texto = text_objects("Ход белых", fonte, BRANCO)
                rect_texto.center = (700, ALTURA / 2)
                display.blit(surface_texto, rect_texto)
            else:
                surface_texto, rect_texto = text_objects("Ход чёрных", fonte, VERMELHO_CLARO)
                rect_texto.center = (700, ALTURA / 2)
                display.blit(surface_texto, rect_texto)
        else:
            surface_texto, rect_texto = text_objects("Игра окончена", fonte, AZUL)
...
def cria_botao(msg, sqr, cor1, cor2, cor_texto, acao=None):
    ...
    fontePequena = pygame.font.SysFont('', 20)
...
def regras():
    ...
    while not sair:
        ...
        info1 = fonte.render('Это класические шашки на поле 8 на 8, друг потив друа.', False, (VERDE_ESCURO))
        info2 = fonte.render('По окончании игры будет выведен результат.', False, (VERDE_ESCURO))
        info3 = fonte.render("Хорошей, честной игры. Удачи!", False, (VERDE_ESCURO))
        info4 = fonte.render("Для возврата на главный экран нажмите любую клавишу мыши", False, (VERDE_ESCURO))
        info5 = fonte.render("В нижней части окна.", False,(VERDE_ESCURO))

        display.blit(info1, (5, 65))
        display.blit(info2, (5, 95))
        display.blit(info3, (5, 125))
        display.blit(info4, (5, 500))
        display.blit(info5, (5, 530))
...
def tela_vencedor(vencedor):
    ...
    while not sair:
        ...
        if vencedor == "Ничья":
            surface_texto, rect_texto = text_objects("Ничья!", fonte, BRANCO)
        elif vencedor == "x":
            surface_texto, rect_texto = text_objects("Победа чёрных", fonte, BRANCO)
        elif vencedor == "o":
            surface_texto, rect_texto = text_objects("Победа белых", fonte, BRANCO)
...
def menu_jogo():
    while True:
        ...
        display.fill(PRETO)
        fonte = pygame.font.SysFont('', 50)
        surface_texto, rect_texto = text_objects("Игра в шашки", fonte, BRANCO)
        rect_texto.center = ((LARGURA / 2), ALTURA / 3)
        display.blit(surface_texto, rect_texto)

        cria_botao("Начать!", (LARGURA - 760, ALTURA / 2, 120, 40), VERDE_CLARO, VERDE_ESCURO, BRANCO, loop_jogo)
        cria_botao("Инструкция", (LARGURA - 455, ALTURA / 2, 120, 40), BRANCO, CINZA, PRETO, regras)
        cria_botao("Выход", (LARGURA - 160, ALTURA / 2, 120, 40), VERMELHO_CLARO, VERMELHO, BRANCO, sair)
